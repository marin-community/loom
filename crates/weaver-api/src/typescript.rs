...
//! checked-in file that could disagree with the document it came from.
//!
//! Deliberately partial: anything outside the constructs schemars actually
//! emits for Loom's DTOs becomes `unknown` rather than a guess.
...
/// Render the whole module: shared types, then one input and output per
/// operation, then the operation table `invokeOperation` is keyed on.
pub fn module() -> String {
    let document = crate::operations::openapi_document(env!("CARGO_PKG_VERSION"));
