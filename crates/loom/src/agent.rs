...
const HANDOFF_SUMMARY_INPUT_MAX_BYTES: usize = 128 * 1024;
const HANDOFF_SUMMARY_OUTPUT_MAX_BYTES: usize = 32 * 1024;

/// Best-effort digest generated by the incoming provider's cheap model. Custom
/// providers have no one-shot/model-tier contract, so they return unavailable
/// instead of silently borrowing a different runtime.
...
struct HandoffSummarizer {
    program: &'static str,
    args: Vec<String>,
    model: &'static str,
}

fn handoff_summarizer(runtime: &str) -> Option<HandoffSummarizer> {
    match runtime {
        "claude" => Some(HandoffSummarizer {
            program: "claude",
            args: [
                "-p",
                "--model",
...
/// Run an isolated, non-persistent summarizer through the provider about to
/// take over. All failures degrade to an unavailable result; provider handoff
/// must remain possible when a binary, credential, or model is exhausted.
pub async fn summarize_handoff(
    ...
    let Some(spec) = handoff_summarizer(runtime) else {
    ...
    let model = Some(spec.model.to_string());
    if prompt.len() > HANDOFF_SUMMARY_INPUT_MAX_BYTES {
        return HandoffSummary {
            text: None,
            model,
            status: "unavailable",
    ...
    // The digest process needs the incoming provider's credentials, but not the
    // live session's authority or lifecycle hooks.
    for key in [
        "WEAVER_API",
        "WEAVER_BRANCH",
        "LOOM_SESSION_ID",
    ...
    let Some(text) = run_handoff_summarizer(command, prompt, timeout).await else {
        return HandoffSummary {
            text: None,
            model,
            status: "unavailable",
...
async fn run_handoff_summarizer(
    ...
    let operation = async {
        ...
        let read_output = async {
            ...
            loop {
                let count = stdout.read(&mut buf).await.ok()?;
                if count == 0 {
                    break;
                }
                let room = HANDOFF_SUMMARY_OUTPUT_MAX_BYTES.saturating_sub(kept.len());
        ...
        let (output, oversized) = output?;
        (status.success() && !oversized).then_some(output)
    };
...
mod tests {
    ...
    #[test]
    fn handoff_summarizer_uses_the_incoming_providers_cheap_model() {
        let claude = handoff_summarizer("claude").unwrap();
        assert_eq!(claude.program, "claude");
        assert_eq!(claude.model, "haiku");
        assert!(claude
            .args
            .windows(2)
            .any(|args| args == ["--model", "haiku"]));
        assert!(claude.args.windows(2).any(|args| args == ["--tools", ""]));
        assert!(claude
            .args
            .iter()
            .any(|arg| arg == "--no-session-persistence"));

        let codex = handoff_summarizer("codex").unwrap();
        assert_eq!(codex.program, "codex");
        assert_eq!(codex.model, "gpt-5.6-luna");
        assert!(codex
            .args
            .windows(2)
            .any(|args| args == ["--model", "gpt-5.6-luna"]));
        ...
        assert!(handoff_summarizer("custom-acp").is_none());
    }

    fn summary_test_command(script: &str) -> tokio::process::Command {
    ...
    #[tokio::test]
    async fn handoff_summarizer_rejects_empty_failed_oversized_and_slow_output() {
        let timeout = Duration::from_secs(2);
        let text = run_handoff_summarizer(
            summary_test_command("printf '  useful digest  '"),
            "history",
            timeout,
        )
        .await;
        assert_eq!(text.as_deref(), Some("useful digest"));

        assert!(
            run_handoff_summarizer(summary_test_command("true"), "history", timeout)
                .await
                .is_none()
        );
        assert!(
            run_handoff_summarizer(summary_test_command("exit 7"), "history", timeout)
                .await
                .is_none()
        );
        assert!(run_handoff_summarizer(
            summary_test_command("head -c 40000 /dev/zero | tr '\\0' x"),
            "history",
            timeout,
        )
        .await
        .is_none());
        assert!(run_handoff_summarizer(
            summary_test_command("sleep 1; printf late"),
            "history",
    ...
    #[tokio::test]
    async fn handoff_summarizer_rejects_input_before_spawning() {
        let result = summarize_handoff(
            "claude",
            &"x".repeat(HANDOFF_SUMMARY_INPUT_MAX_BYTES + 1),
            &[],
