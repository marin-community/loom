...
fn slack_goal(
    ...
    let mut goal = format!(
        "You've been summoned from Slack (channel {channel}) to work on `{repo}`.\n\n\
         ...
         ## {history_heading}\n{history_note}\n\n{history}\n\n\
         ## How to respond\n\
         - Finish by sending a `result` to your own Loom channel; its origin binding delivers the same durable item to this Slack thread. Use `loom.channel_send` with `channel: \"self\"` and `kind: \"result\"`. For a one-off message to another Slack thread already routed to this session, use `loom.messaging_slack_send` with that explicit thread.\n\
