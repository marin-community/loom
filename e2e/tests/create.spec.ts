...
test.describe("creating a session via the UI form", () => {
  ...
  test("keeps arrow navigation inside the repository picker", async ({
    page,
    weaver,
  }) => {
    ...
    await repo.focus();
    const option = page
      .getByTestId("recent-repo")
      .filter({ hasText: weaver.repoPath });
  ...
  test("launches from the workbench with canonical profile and title", async ({
    page,
    weaver,
  }) => {
    ...
    await expect(page.getByTestId("launch-profile-picker")).toHaveValue(
      "ui-launch-plan",
    );
    ...
    await expect(page.getByTestId("scratch-picker-file")).toContainText(
      "trace.log",
    );
    ...
    expect(files).toEqual([
      { name: "trace.log", bytes: Buffer.byteLength("panic at line 42\n") },
    ]);
    ...
    await expect(card).not.toContainText(
      "Inbox / Investigate the attached trace",
    );
    await card.click();
  ...
  test("refreshes added, edited, and deleted profiles without losing the cached draft", async ({
    ...
    await expect(
      page
        .getByTestId("launch-profile-picker")
        .locator('option[value="cached-template"]'),
    ).toHaveCount(1);
    await page
      .getByTestId("launch-profile-picker")
      .selectOption("cached-template");
    await page.getByTestId("override-mode").selectOption("plan");
    ...
    await expect(
      profileDelete.getByRole("button", { name: "Confirm" }),
    ).toBeFocused();
    await page.keyboard.press("Escape");
    await expect(
      profileDelete.getByRole("button", { name: "Delete" }),
    ).toBeFocused();
    await profileDelete.getByRole("button", { name: "Delete" }).click();
