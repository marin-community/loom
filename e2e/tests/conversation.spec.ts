...
function demoLog() {
  ...
  for (let i = 0; i < 10; i++) {
    tasks.push({
      kind: "tool_use",
      name: "TaskCreate",
      input: { title: `subtask ${i + 1}` },
    });
    tasks.push({
      kind: "tool_result",
      output: `Created #${100 + i}`,
      is_error: false,
    });
  }
  return {
    ...
    messages: [
      {
        role: "context",
        blocks: [{ kind: "text", text: "Session primer — follow AGENTS.md." }],
      },
      {
        role: "user",
        timestamp: "2026-06-21T10:00:00.000Z",
        blocks: [{ kind: "text", text: "Add a dark-mode toggle." }],
      },
      {
        role: "assistant",
      ...
      },
      {
        ...
        blocks: [{ kind: "text", text: "Make it persist." }],
      },
      {
        role: "assistant",
        timestamp: "2026-06-21T10:05:01.000Z",
        blocks: [{ kind: "text", text: "It persists via the store." }],
      },
      {
        role: "user",
        timestamp: "2026-06-21T10:09:00.000Z",
        blocks: [{ kind: "text", text: "Ship it." }],
      },
      {
        role: "assistant",
        timestamp: "2026-06-21T10:09:01.000Z",
        blocks: [{ kind: "text", text: "Opening the PR." }],
      },
    ],
  };
}
...
function longLog() {
  ...
  const notes = Array.from({ length: 40 }, (_, i) => ({
    ...
    blocks: [
      {
        kind: "text",
        text: `Progress note ${i + 1}: the change builds and the suite stays green.`,
      },
    ],
  }));
...
test.describe("conversation view", () => {
  test("offers a plain-language catch-up without exposing resumption internals", async ({
    ...
    // The stored read and the on-demand generation are two operations now, so
    // the path — not the method — says which one the page asked for.
    await page.route("**/api/sessions/resumption_cue/*", async (route) => {
      const operands = route.request().postDataJSON() as { session?: string };
      if (operands?.session !== session.id) return route.fallback();
      const generated = new URL(route.request().url()).pathname.endsWith(
        "/ensure",
      );
      await route.fulfill({
        json: {
  ...
  test("opens scrolled to the foot — the newest exchange shows first", async ({
    page,
    weaver,
  }) => {
    ...
    await expect
      .poll(() => footDistance(conv), { timeout: 30_000 })
      .toBeLessThan(120);
    await expect(
      page.getByText("Progress note 40", { exact: false }),
    ).toBeVisible();
  });
  ...
  test("scrolling up releases the pin — a refresh keeps the reader in place", async ({
    ...
    await expect(page.getByText("A fresh reply just landed.")).toBeAttached({
      timeout: 15_000,
    });
    expect(await conv.evaluate((el) => el.scrollTop)).toBeLessThan(300);
  });

  test("folds tool activity and run-length collapses repeated calls", async ({
    page,
    weaver,
  }) => {
    ...
    await expect(
      conv.getByText("rg -n theme src/", { exact: false }),
    ).toBeVisible();
    await expect(conv.getByText("src/theme.ts:12")).toHaveCount(0);
    await page
      .getByTestId("tool-fold")
      .filter({ hasText: "Bash" })
      .getByRole("button")
      .first()
      .click();
  ...
  test("category filters strip tool / thinking noise on demand", async ({
    page,
    weaver,
  }) => {
    await openConversation(page, weaver);
  ...
  test("expand-all opens every fold, collapse-all closes them", async ({
    page,
    weaver,
  }) => {
    await openConversation(page, weaver);
    ...
    await expect(conv.getByText("src/theme.ts:12")).toBeVisible();
    await expect(
      conv.getByText("Let me find the theme store first."),
    ).toBeVisible();
  ...
  test("the prompt index lists every user turn and highlights on jump", async ({
    page,
    weaver,
  }) => {
    await openConversation(page, weaver);
  ...
  test("the composer sends a new prompt to the agent and clears the input", async ({
    ...
    // send → type-into-the-pane primitive that `sessions.send` wraps).
    await expect
      .poll(async () => {
        const res = await fetch(`${weaver.baseUrl}/api/sessions/events/list`, {
          method: "POST",
        ...
        return log.some(
          (e) => e.kind === "nudge" && e.data?.text === "please run the tests",
        );
      })
      .toBe(true);
  });
  ...
  test("phone conversation keeps readable prose and a focus-safe composer", async ({
    ...
    const prose = page
      ...
      .locator("[data-anchor] .markdown-body")
      .first();
    await expect(prose).toHaveCSS("font-size", "15px");
    await expect(page.getByTestId("composer-input")).toHaveCSS(
      "font-size",
      "16px",
    );
  ...
  test("the live status line tracks Working ↔ idle", async ({
    page,
    weaver,
  }) => {
    const s = await openConversation(page, weaver);
  ...
  test("the log auto-refreshes when the agent reaches a turn boundary", async ({
    page,
    weaver,
  }) => {
    ...
    await weaver.hook(s, "idle");
    await expect(page.getByText("A fresh reply just landed.")).toBeVisible({
      timeout: 15_000,
    });
  });
});
