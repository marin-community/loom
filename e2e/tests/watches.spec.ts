...
test.describe("watch panel", () => {
  ...
  test("shows an empty state when there are no watches", async ({
    page,
    weaver,
  }) => {
    await page.goto(`${weaver.baseUrl}/watches`);
  ...
  test("lists a watch and auto-selects it into the detail pane", async ({
    ...
    await expect(row.getByTestId("watch-active-dot")).toHaveAttribute(
      "data-active",
      "false",
    );
    // Never run yet → the outcome badge reads "never run".
    await expect(row.getByTestId("outcome-badge")).toHaveAttribute(
      "data-outcome",
      "none",
    );
  ...
  test("uses the mailbox command grammar for selection, tabs, and creation", async ({
    ...
    await expect(
      help.locator('[data-command-id="watches.cursor-down"]'),
    ).toBeVisible();
    await expect(
      help.locator('[data-command-id="watches.tab.script"]'),
    ).toBeVisible();
    ...
    await expect(page.getByTestId("watch-tab-script")).toHaveAttribute(
      "aria-selected",
      "true",
    );
    await page.keyboard.press("3");
    await expect(page.getByTestId("watch-tab-config")).toHaveAttribute(
      "aria-selected",
      "true",
    );
  ...
  test("shows the builtin script source under the Script tab", async ({
    page,
    weaver,
  }) => {
  ...
  test("creates a watch through the form in the detail pane", async ({
    page,
    weaver,
  }) => {
    await page.goto(`${weaver.baseUrl}/watches`);
  ...
  test("toggles enabled, dry-runs, and surfaces the outcome + the run log", async ({
    ...
    await expect(runRows.first()).toContainText(
      /surveyed 0 sessions in scope/i,
    );
  ...
  test("keeps routine history behind a signal-first digest", async ({
    page,
    weaver,
  }) => {
    ...
    for (let index = 0; index < 3; index += 1) {
      const response = await page.request.post(
        `${weaver.baseUrl}/api/watches/run`,
        {
    ...
    await expect(page.getByTestId("watch-run-digest")).toContainText(
      "2 routine rounds hidden",
    );

    await page.getByTestId("watch-runs-toggle").click();
    await expect(page.getByTestId("watch-run-row")).toHaveCount(3);
    await expect(page.getByTestId("watch-run-digest")).toContainText(
      "showing full round history",
    );
  });

  test("edits the prompt and capabilities under the Config tab", async ({
    page,
    weaver,
  }) => {
    const o = await weaver.seedWatch({
    ...
    await expect(page.getByTestId("watch-prompt")).toContainText(
      "revised judgement",
    );
  ...
  test("surfaces warm-session state under the Config tab", async ({
    page,
    weaver,
  }) => {
  ...
  test("deletes a custom watch; builtin watches offer no delete", async ({
    page,
    weaver,
  }) => {
