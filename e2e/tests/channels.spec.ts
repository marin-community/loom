...
test.describe('channels mailbox', () => {
  test('shows session channels, typed status, read acknowledgement, and delivery receipts', async ({
    ...
    const before = await page.request.post(`${weaver.baseUrl}/api/channels/list`, {
      data: {},
    });
    ...
    await expect(page.locator(`[data-channel-id="${session.id}"]`)).toContainText('Channel work');
    ...
    await expect
      .poll(async () => {
        const response = await page.request.post(`${weaver.baseUrl}/api/channels/list`, {
          data: {},
        });
        ...
        return rows.find((channel) => channel.id === session.id)?.unread_urgent_count;
      })
      .toBe(0);
    ...
    await page.getByTestId('channel-create-topic').fill('coordinate the release');
