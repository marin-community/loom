...
test.describe('staged artifact reviews', () => {
  test('drafts, conflicts, re-anchors, and submits one coherent review', async ({
    ...
    const alphaResponse = page.waitForResponse(
      (response) =>
        ...
        (response.request().postDataJSON() as { name?: string })?.name === 'alpha',
    );
    ...
    await expect(page.locator('.markdown-body h1')).toContainText('Beta identity');
    releaseAlpha();
    ...
    await composer.locator('textarea').fill('Keep the document title aligned with the review.');
    ...
    await firstComment;
    const initialReviews = await page.request.post(`${weaver.baseUrl}/api/reviews/list`, {
      data: {
    ...
    const secondComment = await page.request.post(`${weaver.baseUrl}/api/reviews/comments/create`, {
      data: {
        id: initialDraft.id,
    ...
    await anchored.getByTestId('review-comment-edit').fill('Explain prefix and suffix recovery.');
    ...
    await page.reload();
    await page.getByTestId('review-tray-toggle').click();
    await expect(page.getByTestId('review-overall-note')).toHaveValue('Initial overall guidance.');
    await expect(
      page
        .locator('[data-testid^="review-comment-"]')
    ...
    const listed = await page.request.post(`${weaver.baseUrl}/api/reviews/list`, {
      data: {
        subject_kind: 'artifact',
        subject_key: 'design',
        session: session.id,
    ...
    const draft = ((await listed.json()) as Array<{ id: number; draft_revision: number }>)[0];
    const external = await page.request.post(`${weaver.baseUrl}/api/reviews/update`, {
      data: {
        id: draft.id,
    ...
    const conflict = page.waitForResponse(
      (response) =>
        response.status() === 409 && new URL(response.url()).pathname === '/api/reviews/update',
    );
    await page.getByTestId('review-overall-note').fill('Conflicting stale guidance.');
    ...
    await page.getByTestId('review-overall-note').fill('Address this before landing.');
    await page.getByTestId('review-overall-note').blur();
    await finalSave;

    const revised = DOC.replace('How do anchors survive', 'How can anchors survive');
    ...
    await tray.getByRole('button', { name: /Explain prefix and suffix recovery/ }).click();
    ...
    const submit = page.waitForRequest(
      (request) =>
        request.method() === 'POST' && new URL(request.url()).pathname === '/api/reviews/submit',
    );
  ...
  test('preserves a long document and in-flight overall draft through pop and dock', async ({
    ...
    await expect(page.getByTestId('review-surface')).toHaveAttribute('inert', '');
    await expect(page.getByTestId('review-overall-note')).toBeDisabled();
    const frozenValue = await page.getByTestId('review-overall-note').inputValue();
    await page
      .getByTestId('review-overall-note')
      .evaluate((element: HTMLTextAreaElement) => element.focus());
    await page.keyboard.type('must not enter the frozen review');
    await expect(page.getByTestId('review-overall-note')).toHaveValue(frozenValue);
    ...
    await expect(page).toHaveURL(`${weaver.baseUrl}/s/${session.id}/artifacts/other`);
    await expect(page.locator('.markdown-body h1')).toContainText('Other artifact');
    await page.goBack();
    await expect(page).toHaveURL(`${weaver.baseUrl}/s/${session.id}/artifacts/design`);
    ...
    page.on('request', (request) => {
      if (request.method() === 'POST' && /\/api\/reviews\/\d+\/submit$/.test(request.url())) {
    ...
    const reviewsResponse = await page.request.post(`${weaver.baseUrl}/api/reviews/list`, {
      data: {
        subject_kind: 'artifact',
        subject_key: 'design',
        session: session.id,
    ...
    const submitResponse = await page.request.post(`${weaver.baseUrl}/api/reviews/submit`, {
      data: {
        id: review.id,
    ...
    patchGate.resolve();
    await expect(page.getByRole('alert')).toContainText('injected route save failure');
    await expect(page).toHaveURL(artifactUrl);
    ...
    await expect(reviewNote).toHaveValue('Route feedback survives app-rail navigation.');
  });
});
