...
test.describe("settings · custom agents", () => {
  test("add a custom agent, see it in the picker, then delete it", async ({
    page,
    weaver,
  }) => {
    ...
    await page.screenshot({
      path: "/tmp/custom-agents-dark.png",
      fullPage: true,
    });
    await page.evaluate(() => {
    ...
    await page.screenshot({
      path: "/tmp/custom-agents-light.png",
      fullPage: true,
    });
    await page.evaluate(() => {
    ...
    await expect(
      panel.locator("li").filter({ hasText: "Aider" }),
    ).toBeVisible();
    // ...and becomes selectable by the default profile.
    ...
    await expect(agent.locator("option", { hasText: "Aider" })).toHaveCount(0);
  });
});
